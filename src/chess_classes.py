class Board:
    ...
    def update(self, start, end, update_state=True) -> bool:
        """This method updates the board state after a move is made.
...
class Game:
    ...
    def play(self) -> None:
        ...
        while True:
            if self.turn == 'W':
                print(f'White to move on Move {self.move_number}')
            else:
                print(f'Black to move on Move {self.move_number}')
            ...
            if self.board.check_if_check(self.turn):

                if self.turn == 'W':
                    self.board.state[self.board.wk_pos[0]][self.board.wk_pos[1]].is_checked = True
                else:
                    self.board.state[self.board.bk_pos[0]][self.board.bk_pos[1]].is_checked = True

                # checkmate if no available moves
                if not self.board.is_there_a_move(self.turn):
                    print(self.board)
                    if self.turn == 'W':
            ...
            # if the move is not a check
                ...
                if self.turn == 'W':
                    self.board.state[self.board.wk_pos[0]][self.board.wk_pos[1]].is_checked = False
                else:
                    self.board.state[self.board.bk_pos[0]][self.board.bk_pos[1]].is_checked = False
